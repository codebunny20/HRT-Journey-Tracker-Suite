...
DATA_FILENAME = "entries.jsonl"
STORAGE_DIRNAME = "storage"  # <-- add
...
def _ensure_storage_ready() -> str:
    """
    Ensures ./storage exists next to this script and entries.jsonl exists.
    Returns full path to entries.jsonl.
    """
...
class ViewEntriesDialog(QDialog):
    ...
    def _load_entries(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return []
        # newest-first (timestamp_local is sortable)
        entries.sort(key=lambda e: e.get("timestamp_local", ""), reverse=True)
        return entries
    ...
    def _delete_selected(self):
        ...
        path = _app_data_path()
        target_line = json.dumps(entry, ensure_ascii=False)

        try:
            with open(path, "r", encoding="utf-8") as f:
                lines = f.readlines()
        except OSError as e:
            QMessageBox.critical(self, "Delete failed", f"Could not read:\n{path}\n\n{e}")
            return

        # Remove the first matching JSON line (ignoring trailing newline differences)
        removed = False
        new_lines: List[str] = []
        for line in lines:
            if (not removed) and (line.strip() == target_line):
                removed = True
                continue
            new_lines.append(line)

        if not removed:
            QMessageBox.warning(self, "Not found", "Could not find the selected entry in the file (it may have changed).")
            self._refresh()
            return
...
class HelpDialog(QDialog):
    def __init__(self, parent=None):
        ...
        text.setPlainText(
            ...
            "\n"
            "Data location\n"
            "- Entries are stored in a JSON Lines file in the app’s storage folder.\n"
...
class MainWindow(QMainWindow):
    ...
    def _save_entry(self):
        ...
        record: Dict[str, Any] = {
            "timestamp_local": dt.strftime("%Y-%m-%d %H:%M"),
