...
This workspace contains multiple small Windows desktop apps being developed in parallel:

- **Journey Journal** (`Journey Journal/`) — journaling-focused app
- **TrackMyHRT** (`TrackMyHRT/`) — HRT tracking-focused app
- **Resource manager** (`Resource manager/`) — save/open/copy/search helpful links (bookmarks list)
- **Launcher** (`Launcher/`) — workspace app picker / launcher

All apps are separate, iterative projects. The goal is to experiment, refine, and stabilize features independently, then merge or consolidate them into a single finished application once the direction is clear.

- **Target environment:** Windows + Python launcher (`py`) + VS Code
- **UI:** PySide6
- **Packaging:** PyInstaller (each app may have its own `.spec` file and assets)
...
├─ Launcher/
│  ├─ Launcher.py
│  ├─ (optional) Launcher.spec
│  └─ assets/
│     └─ (optional) launcher.ico
└─ README.md                   # this file
```

**Notes:**
- Each app is self-contained (some include a `data/` package for UI/storage code).
...
### Run Resource manager (dev)

```powershell
py ".\Resource manager\main.py"
```
...
- **Auto-discovers apps** by scanning workspace root subfolders (depth up to **2**).
- Shows a simple **Apps** panel with one button per discovered app.
- Tracks **Recent activity** (“last opened”) via **QSettings**.
- Runs quick **diagnostics** before launching:
  - `python -m py_compile <script>` (syntax / import-time errors)
  - `python -c "import PySide6"` (missing dependency check)
- Supports **per-app Alt+<letter> shortcuts** (auto-assigned).
- Supports **theme toggling** (dark/light) and persists it in settings.
- Can prompt before starting **duplicate instances** of the same app.
...
## Development approach (why multiple apps)
...
- **Resource manager** focuses on saving and quickly opening commonly used links/resources
